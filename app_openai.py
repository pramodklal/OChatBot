        ...
        try:
            ...
            if "429" in str(e) or "quota" in str(e).lower() or "rate_limit" in str(e).lower():
                if attempt < max_retries - 1:
                    ...
                    raise Exception(f"Rate limit exceeded after {max_retries} attempts. Please wait and try again later, or check your API plan.")
            else:
                raise e
        ...
        try:
            ...
            if "429" in str(e) or "quota" in str(e).lower() or "rate_limit" in str(e).lower():
                if attempt < max_retries - 1:
                    ...
                    raise Exception(f"API rate limit exceeded. Please wait for quota reset or upgrade your plan.")
            else:
                raise e
...
def main():
    st.set_page_config(
        page_title="Multi-Files Chatbot using OpenAI",
        #page_icon="🤖"
        page_icon=":books:"
    )
    ...
    with st.sidebar:
        ...
        if (model_list[1]=="GPT-4"):
            selected_model= "gpt-4"
        if (model_list[1]=="GPT-4-Turbo"):
            selected_model = "gpt-4-turbo-preview"
        if (model_list[1]=="GPT-3.5-Turbo"):
    ...
    st.markdown("<h2 style='color:purple;vertical-align:top;'>Multi-Files Chatbot using OpenAI </h2>", unsafe_allow_html=True)
    ...
    st.markdown('<div style="text-align:center; color: #00008B; font-size: 1.1em; font-weight: bold;">Design & Developed by Code Insights @pramodklal</div>', unsafe_allow_html=True)
